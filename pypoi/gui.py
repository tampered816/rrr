...
class PoissonBlendingApp(tkinter.Tk):
    NUM_OF_EXAMPLES = 210  # Examples in the testimages folder.
    ...
    def create_widgets(self):
        ...
        tkinter.Button(action_frame, text=u'i', command=self.show_dst_mask).pack()
    ...
    def load_example(self, example_id):
        ...
        def _load_example():
            src_path = rp('./testimages/%d.jpg' % example_id)
            dst_path = rp('./testimages/%d.jpg' % example_id)
            mask_path = None
            ...
            try:
                ...
                mask_path = rp('./testimages/mask/%d-mask-alpha.jpg' % example_id)
            except IndexError:
                pass
    ...
    def show_dst_mask(self):
        mask = self.dst_img_manager.src_img_manager.image_mask
        mask.show()
        print(mask)
        return
        result_window = tkinter.Toplevel()
        label = tkinter.Label(result_window, image=mask)
        label.image = self.image_tk_result  # for holding reference counter
        label.pack()
