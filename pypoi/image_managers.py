...
class SourceImageManager(ImageManager):
    ...
    """
    EDIT_MODES = (
        EditMode('Draw back', 'draw'),
        EditMode('Draw front', 'draw2'),
    ...
    def __init__(self):
        ...
        self.edit_mode = None
        self.rotate = None # useless for src
    ...
    def on_mouse_down(self, event):
        ...
        elif self.edit_mode.get() == 'draw2':
            self.modify_mask(event.x, event.y, 150)
...
class DestinationImageManager(ImageManager):
    ...
    def imagepre(self):
        ...
        imagepre=self.draw_mask(image)

        return imagepre
    def resize(self, new_scale):
        self.image = PIL.Image.open(self.path).convert("RGB")
        new_size = tuple([int(x * new_scale) for x in self.image.size])
        self.image = self.image.resize(new_size)
        self.draw()

    def draw_full_mask(self):
        self.full_mask = PIL.Image.new("RGB", (self.image.size[0], self.image.size[1]))
        img_src = self.src_img_manager.image_src.copy()
        mask = self.src_img_manager.image_mask.copy()
        squared_msk_img = SquareMaskImage(img_src, mask, self.offset, self.rotate)
        self.full_mask.paste(
            squared_msk_img.squared_msk_img,
            squared_msk_img.offset
        )
        # self.full_mask.show()
