...
def main(args=None):
    ...
    ckpt = torch.load(args.resume, pickle_module=pickle)
    if args.not_strict:
...
def foo(stage=1, alphaDir='', crop_or_resize='whole', cuda=False, max_size=1600, not_strict=False, size_h=320, size_w=320):
    class Dict(dict):
        __setattr__ = dict.__setitem__
        __getattr__ = dict.__getitem__
    args = Dict()
    args.imgDir='matting/data/image/'
    args.trimapDir='matting/data/trimap/'
    args.saveDir='matting/data/pred/'
    args.resume='matting/model/stage1_skip_sad_52.9.pth'
    args.alphaDir=alphaDir
    args.crop_or_resize=crop_or_resize
    args.cuda=cuda
    args.max_size=max_size
    args.not_strict=not_strict
    args.size_h=size_h
    args.size_w=size_w
    args.stage=stage
